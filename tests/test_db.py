...
class DatabaseMigrationTests(unittest.TestCase):
    ...
    def test_existing_database_receives_job_result_column(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(versions, set(range(1, 39)))
            with db.connect() as upgraded:
                tables = {
                    row["name"]
                ...
                session_columns = {
                    row["name"]
                    for row in upgraded.execute("PRAGMA table_info(auth_sessions)")
                }
            ...
            self.assertIn("device_export_tickets", tables)
            self.assertIn("device_roster_groups", tables)
            self.assertIn("device_sync_runs", tables)
            self.assertIn("mobile_installations", tables)
            self.assertIn("mobile_push_preferences", tables)
            self.assertIn("mobile_push_outbox", tables)
            self.assertIn("mobile_releases", tables)
